...
EventMan.prototype.initMonth = function(rootNode){
  ...
  for (var i = 0; i < allEvents.length; i++) {
    ...
    var eventObj = new EventObject();
		eXo.cs.EventManager.addEvent(allEvents[i],"mouseover",eXo.calendar.EventTooltip.show);
		eXo.cs.EventManager.addEvent(allEvents[i],"mouseout",eXo.calendar.EventTooltip.hide);
  ...
  if (!moreContainerNode.style.display || moreContainerNode.style.display == 'none') {
    moreContainerNode.style.display = 'block';
		...
		moreContainerNode.onmousedown = function(evt){
			...
			if(eXo.cs.EventManager.getMouseButton(evt) == 2) eXo.core.DOMUtil.hideElementList.remove(this);
		}

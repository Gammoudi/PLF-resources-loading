...
UICalendarPortlet.prototype.attachSwapClass = function(compId,className,hoverClass){
    ...
    var i = items.length;
    while(i--){
    	eXo.cs.EventManager.addEvent(items[i],"mouseover",function(){
    		eXo.cs.Utils.swapClass(this,hoverClass);
    	});
    	eXo.cs.EventManager.addEvent(items[i],"mouseout",function(){
    		eXo.cs.Utils.swapClass(this,hoverClass);
    	});
    };
} ;
...
UICalendarPortlet.prototype.showHide = function(obj){
    if (obj.style.display != "block") {
        ...
        eXo.cs.EventManager.addEvent(obj,"mouseover",eXo.calendar.UICalendarPortlet.autoHide);
        eXo.cs.EventManager.addEvent(obj,"mouseout",eXo.calendar.UICalendarPortlet.autoHide);
...
//    this.setFocus(this.viewer, EventDayContainer);
    ...
    for (var i = 0; i < el.length; i++) {
        this.setSize(el[i]);
        eXo.cs.EventManager.addEvent(el[i],"mouseover",eXo.calendar.EventTooltip.show);
				eXo.cs.EventManager.addEvent(el[i],"mouseout",eXo.calendar.EventTooltip.hide);
				eXo.cs.EventManager.addEvent(el[i],"mousedown",eXo.calendar.UICalendarPortlet.initDND);
        eXo.cs.EventManager.addEvent(el[i],"dblclick",eXo.calendar.UICalendarPortlet.ondblclickCallback);
        marker = eXo.core.DOMUtil.findFirstChildByClass(el[i], "div", "ResizeEventContainer");
        eXo.cs.EventManager.addEvent(marker,"mousedown",eXo.calendar.UIResizeEvent.init);
    }
    this.items = el;
...
UICalendarPortlet.prototype.listViewDblClick = function(form){
	...
	var chk = null ;
	while(i--){
		eXo.cs.EventManager.addEvent(tr[i],"dblclick",this.listViewDblClickCallback);
	}
};
...
UISelection.prototype.start = function(evt){
    try {
        var UISelection = eXo.calendar.UISelection;
        var src = eXo.cs.EventManager.getEventTarget(evt);
        if ((src == UISelection.block) || (eXo.cs.EventManager.getMouseButton(evt) == 2) || (eXo.core.DOMUtil.hasClass(src,"TdTime"))) {
						return;
        }
  ...
  UTC_0: "UTC:0",
	...
	getContainer: function(evt){
		...
		if(!self._container){
			...
			eXo.cs.EventManager.addEvent(self._container,"mouseover",function(evt){
				self.cleanupTimer(evt);
			});
			eXo.cs.EventManager.addEvent(self._container,"mouseout",function(evt){
				self.hide(evt);
			});
			eXo.cs.EventManager.addEvent(self._container,"click",function(evt){
				self.hideElement();

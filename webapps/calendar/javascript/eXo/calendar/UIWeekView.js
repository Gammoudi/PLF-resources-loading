...
UIWeekView.prototype.init = function() {
  ...
  //UICalendarPortlet.setFocus() ;
	...
	for(var i = 0 ; i < len ; i ++){		
		...
		eXo.cs.EventManager.addEvent(this.items[i],"mousedown",UIWeekView.dragStart);
		eXo.cs.EventManager.addEvent(this.items[i],"dblclick",eXo.calendar.UICalendarPortlet.ondblclickCallback);
		eXo.cs.EventManager.addEvent(this.items[i],"mouseover",eXo.calendar.EventTooltip.show);
		eXo.cs.EventManager.addEvent(this.items[i],"mouseout",eXo.calendar.EventTooltip.hide);
		marker = DOMUtil.findFirstDescendantByClass(this.items[i], "div", "ResizeEventContainer") ;
		eXo.cs.EventManager.addEvent(marker,"mousedown",UIWeekView.initResize);
	}
...
UIWeekView.prototype.initAllday = function() {
	var EventManager = eXo.cs.EventManager;
	var UIWeekView = eXo.calendar.UIWeekView ;
...
UIWeekView.prototype.setWidth = function(events) {
	...
	for(var i = 0 ; i < len ; i ++) {
		...
		eXo.cs.EventManager.addEvent(events[i],"mousedown",this.initAlldayDND) ;
	}
	return events ;
} ;
// Resize horizontal

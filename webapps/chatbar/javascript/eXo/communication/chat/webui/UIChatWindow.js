...
UITabControl.prototype.userLeftRoomEventFired = function(user) {
  ...
  var msgBuf = "${chat.message.room.user.left}".replace('{0}', fullName);
  this.writeMsg("${chat.message.system.info}", msgBuf);
...
UITabControl.prototype.userJoinRoomEventFired = function(user) {
  ...
  var msgBuf = "${chat.message.room.user.join}".replace('{0}', fullName);
  this.writeMsg("${chat.message.system.info}", msgBuf);
  // Decode username
...
UITabControl.prototype.fileTransportRequestEventFire = function(FTReqEvent) {
  ...
  var alertContent = "${chat.message.file.transport.request}".replace('{0}', fullName);
...
UITabControl.prototype.fileTransportResponseEventFire = function(FTResEvent) {
  ...
  switch (FTResEvent.status) {
    case 'complete':
      msgContent = "${chat.message.file.transport.response.completed}".replace('{0}', FTResEvent.fileName);
      break;
    case 'error':
      msgContent = "${chat.message.file.transport.response.denied}".replace('{0}', FTResEvent.fileName);
      msgContent += ' ';
      msgContent += "${chat.message.file.transport.response.receiver.offline}";
      break;
    default :
      msgContent = "${chat.message.file.transport.response.denied}".replace('{0}', FTResEvent.fileName);
      break;
...
UITabControl.prototype.fileEventTimeout = function(fileTransportNode, uiTabControlObj) {
  ...
  uiTabControlObj.writeMsg("${chat.message.system.info}", "${chat.message.file.event.time.out}");
...
UITabControl.prototype.initUI = function(buddyId) {
  ...
  if(fullName.indexOf('@') > -1)
    ...
    fullName = "${chat.message.administrative.message}";
...
UIChatWindow.prototype.sendFile = function(fileChooserNode, event) {
  ...
  var uploadForm = eXo.core.DOMUtil.findAncestorByTagName(fileChooserNode, 'form');
    ...
    //eXo.communication.chatbar.webui.UIChatWindow.insertCustomMsg('File exchange: Waiting for authorize...', activeTabControl.tabId);
  	eXo.communication.chatbar.webui.UIChatWindow.insertCustomMsg("${chat.message.file.exchange.waiting.for.authorize}", activeTabControl.tabId);
  	this.onload = null;
  };
	uploadForm.submit();
	...
	eXo.communication.chatbar.webui.UIChatWindow.insertCustomMsg("${chat.message.file.exchange.uploading.file.to.server}", activeTabControl.tabId);

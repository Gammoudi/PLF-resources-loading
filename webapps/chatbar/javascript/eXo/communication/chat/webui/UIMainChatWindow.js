...
UIMainChatWindow.prototype.processErrorAction = function(requestObj, action) {
  ...
  if (requestObj.status == 400
    ...
    window.alert("${chat.message.xmpp.session.is.null}");
    return;
  }
  switch (action) {
  case this.JOIN_TO_ROOM_ACTION:
    ...
    case 401:
      ...
      // again later.');
      window.alert("${chat.message.room.password.error}");
      break;
    case 403:
    case 404:
      window.alert("${chat.message.room.not.unlocked.error}");
      break;
    case 407:
      window.alert("${chat.message.room.user.not.member}");
      break;
    case 409:
    default:
      break;
  ...
  case this.CREATE_ROOM_ACTION:
    ...
    case 407:
      ...
      break;
    case 500:
      window.alert("${chat.message.room.creation.error}");
      break;
    case 409:
    default:
      break;
  ...
  case this.GET_ROOM_CONFIG_ACTION:
    ...
    window.alert("${chat.message.room.configuration.error}");
    break;
  // case this.GET_ROOM_INFO_ACTION:
  case this.GET_ROOM_LIST_ACTION:
  case this.GET_JOINED_ROOM_LIST_ACTION:
...
UIMainChatWindow.prototype.notificationListener = function (eventObj) {
  ...
  if (msg == "session-expired") {
    ...
    this.isExpired = true;
    if (window.confirm("${chat.message.xmpp.session.has.expired}")) location.reload(true);
    
    eXo.cs.EventManager.addEvent(chatbar, "mouseover", function(e) {
      var current = eXo.cs.EventManager.getEventTargetByClass(e, "StatusArea") || 
                    eXo.cs.EventManager.getEventTargetByClass(e, "RoomArea") || 
                    eXo.cs.EventManager.getEventTargetByClass(e, "ContactArea");
      if (current) {
        if (window.confirm("${chat.message.xmpp.session.has.expired}")) location.reload(true);
      }
    });
    ...
    bottomInformation.innerHTML = "${chat.information.session.has.expired}";
    
    var chatwindow = eXo.core.DOMUtil.findFirstDescendantByClass(this.rootNode.parentNode, 'div', 'UIWindow');
    eXo.cs.EventManager.addEvent(chatwindow, "mouseover", function (e) {
      var current = eXo.cs.EventManager.getEventTargetByClass(e, "UIActionbar") && 
                    (eXo.cs.EventManager.getEventTargetByClass(e, "ExportMessageIcon") ||
                     eXo.cs.EventManager.getEventTargetByClass(e, "SendFileIcon") ||
                     eXo.cs.EventManager.getEventTargetByClass(e, "HistoryIcon") ||
                     eXo.cs.EventManager.getEventTargetByClass(e, "OptionsIcon") ||
                     eXo.cs.EventManager.getEventTargetByClass(e, "LeaveIcon") ||
                     eXo.cs.EventManager.getEventTargetByClass(e, "AddChatIcon")
                    );
      if (current) {
        if (window.confirm("${chat.message.xmpp.session.has.expired}")) location.reload(true);
      }
  ...
  if (serverData.subscriptions) {
    ...
    for ( var i = 0; i < subscriptions.length; i++) {
      ...
      case 'subscribe':
        ...
        var str = "${chat.message.confirm.allow.to.see.status}";
  ...
  if (serverData.mucEvents) {
    ...
    for ( var i = 0; i < mucEvents.length; i++) {
      ...
      case this.MUC_ACTION_INVITE_ROOM:
        ...
        // roomName + '"';
        var msgBuf = "${chat.message.room.invite.to.join}"
            .replace('{0}', fullName);
...
UIMainChatWindow.prototype.buddyItemActionCallback = function(event) {
  var buddyNode = eXo.cs.EventManager.getEventTarget(event);// event.srcElement
                                                              // ||
                                                              // event.target;
  eXo.cs.EventManager.cancelEvent(event);
  buddyNode = eXo.cs.DOMUtil.findAncestorByClass(buddyNode, 'TitleIconChat');
  if (!buddyNode) {
    return;
  }
...
UIMainChatWindow.prototype.removeContact = function(buddyId) {
  var str = "${chat.message.confirm.remove.buddy}";
  ...
  if (askPassword) {
    if (askPassword == true) {
      password = window.prompt(
          "${chat.message.room.secret.key.to.access}", '');
      if (!password) {
        return;
      }
    } else {
